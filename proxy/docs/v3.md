...
| `categories` (reverse only) | `stopPlaceTypes` | NeTEx stop place types; see below |

`stopPlaceTypes` takes NeTEx stop place types (`railStation`, `airport`, ...) and restricts results to stop places of those types, excluding other layers - same semantics as v2 `categories`. The v2 layer-like category values (`street`, `vegadresse`, `GroupOfStopPlaces`, `poi`) map to [layers](#layers) instead.
