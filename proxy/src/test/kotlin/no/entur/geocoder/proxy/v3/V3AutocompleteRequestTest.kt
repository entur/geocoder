...
class V3AutocompleteRequestTest {
    ...
    fun `photonZoom returns null when no focus point`() {
        val req = V3AutocompleteRequest(query = "oslo")
    ...
    fun `photonZoom uses default radius of 50km`() {
        val req = V3AutocompleteRequest(query = "oslo", lat = 59.9, lon = 10.7)
    ...
    fun `photonZoom with explicit radius`() {
        ...
        val req = V3AutocompleteRequest(query = "oslo", lat = 59.9, lon = 10.7, radius = 4.0)
    ...
    fun `photonZoom with small radius`() {
        ...
        val req = V3AutocompleteRequest(query = "oslo", lat = 59.9, lon = 10.7, radius = 0.25)
        assertEquals(18, req.photonZoom())
    }

    @Test
    fun `photonZoom clamps to valid range`() {
        val large = V3AutocompleteRequest(query = "oslo", lat = 59.9, lon = 10.7, radius = 100000.0)
        assertEquals(0, large.photonZoom())

        val tiny = V3AutocompleteRequest(query = "oslo", lat = 59.9, lon = 10.7, radius = 0.001)
    ...
    fun `photonLocationBiasScale with explicit weight`() {
        val noFocus = V3AutocompleteRequest(query = "oslo", lat = 59.9, lon = 10.7, weight = 0.0)
        assertEquals(1.0, noFocus.photonLocationBiasScale())

        val maxFocus = V3AutocompleteRequest(query = "oslo", lat = 59.9, lon = 10.7, weight = 1.0)
        assertEquals(0.0, maxFocus.photonLocationBiasScale())

        val half = V3AutocompleteRequest(query = "oslo", lat = 59.9, lon = 10.7, weight = 0.5)
        assertEquals(0.5, half.photonLocationBiasScale())
    }

    @Test
    fun `photonLocationBiasScale clamps weight`() {
        val over = V3AutocompleteRequest(query = "oslo", lat = 59.9, lon = 10.7, weight = 2.0)
        assertEquals(0.0, over.photonLocationBiasScale())

        val under = V3AutocompleteRequest(query = "oslo", lat = 59.9, lon = 10.7, weight = -1.0)
