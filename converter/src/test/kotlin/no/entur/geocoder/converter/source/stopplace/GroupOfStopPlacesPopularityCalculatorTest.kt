...
class GroupOfStopPlacesPopularityCalculatorTest {
    ...
    fun `single member group has boosted popularity`() {
        ...
        val popularity = calculator.calculatePopularity(memberPopularities)
    ...
    fun `multiplication causes exponential growth with more members`() {
        val twoMembers = calculator.calculatePopularity(listOf(60L, 60L))
        val threeMembers = calculator.calculatePopularity(listOf(60L, 60L, 60L))
        val fourMembers = calculator.calculatePopularity(listOf(60L, 60L, 60L, 60L))
    ...
    fun `popularity ordering preserved for increasing member count`() {
        ...
        val popularities = groups.map { calculator.calculatePopularity(it) }
    ...
    fun `single member group applies boost factor correctly`(memberPop: Long, expected: Double) {
        val popularity = calculator.calculatePopularity(listOf(memberPop))
    ...
    fun `two member group multiplies popularities correctly`(pop1: Long, pop2: Long, expected: Double) {
        val popularity = calculator.calculatePopularity(listOf(pop1, pop2))
        assertEquals(expected, popularity)
    }
}
